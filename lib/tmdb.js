...
class Tmdb {
  ...
  syncTV(json, tvPath, callback) {    
    ...
    if (this._configuration.progress) {
      process.stdout.write("Sync",(json.name || Path.basename(tvPath)),"                                \r");
    }
...
//          console.log(res);
  ...
  syncSeason(json, season, tvPath, callback) {
    var tasks=[];
    if (this._configuration.progress) {
      process.stdout.write("Sync",json.tvInfo.name,"season #",season.season_number,"                     \r");
    }
...
//  debug("metasDir=",metasDir);
    ...
    fs.stat(metasDir, (error, stats) => {
      ...
      fs.stat(localPath, (error, localStats) => {
        ...
        if (this._configuration.progress) {
          process.stdout.write("GetImage",p,"                                \r");
        }
